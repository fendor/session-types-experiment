...
par (Process x) (Process y) = Process $ do
  res <- newEmptyTMVarIO
  res' <- newEmptyTMVarIO
...
instance
  Subeffect Process env env' =>
  Subeffect Process ((c :-> s) ': env) ((c :-> s :+ t) ': env')
  where
  sub (Process p) = Process p

instance
  Subeffect Process env env' =>
  Subeffect Process ((c :-> t) ': env) ((c :-> s :+ t) ': env')
  where
  sub (Process p) = Process p
...
instance
  Subeffect Process env env' =>
  Subeffect Process ((c :-> s) ': env) ((c :-> s) ': env')
  where
  sub (Process p) = Process p

instance
  Subeffect Process env env' =>
  Subeffect Process env ((c :-> End) ': env')
  where
  sub (Process p) = Process p
...
affineFix ::
  ...
  Process '[c :-> ToFix h] b
affineFix f x =
  let (Process p) = f (\x' -> let (Process y) = affineFix f x' in Process y) x
   in Process p
